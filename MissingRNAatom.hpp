    ...
    vector<size_t> &nonstd_atom_vec, const int option=0)
{
    ...
    for (it=ideal_rna[residue.resn].begin(); it!=ideal_rna[residue.resn].end(); it++)
    {
        ...
        if (option<6&&found==false) missing_atom_vec.push_back(it->first);
        else if(option==6&&found==false){
            if(it->first==" OP1"||it->first==" OP2"
             ||it->first==" O4'"||it->first==" C2'"
             ||it->first==" O2'"||it->first==" C1'") missing_atom_vec.push_back(it->first);
        }else if(option==7&&found==false){
            if(it->first==" P  "||it->first==" O5'"
             ||it->first==" C5'"||it->first==" C4'"
             ||it->first==" C3'"||it->first==" O3'") missing_atom_vec.push_back(it->first);
        }
    }
    // terminal output that lists missing atoms
    if (missing_atom_vec.size()&&option!=6)
    {
        if(residue.icode!='0'){
            cout<<residue.resn<<residue.resi<<residue.icode<<"\t"
    ...
    const int option=0)
{
    ...
    for (r=0;r<pdb_entry.chains[c].residues.size();r++)
    {
        ...
        else if (option==5)
        {
            ...
            {
                remove_residue_vec.push_back(r);
                if(pdb_entry.chains[c].residues[r].icode!='0'){
                    cout<<"remove residue "
                    <<pdb_entry.chains[c].residues[r].resn
                    <<pdb_entry.chains[c].residues[r].resi
                    <<pdb_entry.chains[c].residues[r].icode
                    <<" without standard atom"<<endl;
                    pdb_entry.chains[c].residues[r].icode='0';
                }
            }
    ...
    if (remove_residue_vec.size())
    {
        ...
        {
            if (pdb_entry.chains[c].residues[i].icode!='0') continue;
            for (j=i+1;j<pdb_entry.chains[c].residues.size();j++)
            {
                if (pdb_entry.chains[c].residues[j].icode=='0') continue;
                ...
                pdb_entry.chains[c].residues[j].icode='0';
                break;
            }
        }
