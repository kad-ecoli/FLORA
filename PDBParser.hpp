/* Purpose1: parse PDB file into data structure similar to Bio.PDB in biopython
    ...
    const int atomic_detail=2,const int allowX=1)
{
        ...
        filename_str.substr(filename_str.length()-18,18)=="-pdb-bundle.tar.gz")
    {
        ...
        chain.residues.shrink_to_fit();
        residue.atoms.clear();
        residue.atoms.shrink_to_fit();
        chainIDmap.clear();
        map<char,string>().swap(chainIDmap);
        PDBmap.clear();
        map<string,map<char,string> >().swap(PDBmap);
        return pep;
    }
...
string write_pdb_structure(ChainUnit &chain,int &i)
{
    ...
    for (r=0;r<chain.residues.size();r++)
    {
	    ...
	    stringstream coor_y;
	    stringstream coor_z;
	    coor_x<<setiosflags(ios::fixed)<<setprecision(3)
		    <<setw(8)<<chain.residues[r].atoms[a].xyz[0];
	    coor_y<<setiosflags(ios::fixed)<<setprecision(3)
		    <<setw(8)<<chain.residues[r].atoms[a].xyz[1];
	    coor_z<<setiosflags(ios::fixed)<<setprecision(3)
		    <<setw(8)<<chain.residues[r].atoms[a].xyz[2];
	    string resi_x=coor_x.str().substr(0,8);
	    string resi_y=coor_y.str().substr(0,8);
	    string resi_z=coor_z.str().substr(0,8);
	    coor_x.clear();coor_x.str("");
	    coor_y.clear();coor_y.str("");
	    coor_z.clear();coor_z.str("");
...
/* Reorder final output into standard PDB atomic order. 
   ONLY TO REODRER RESIDUES WITH FULL ATOMS. */
void standardize_pdb_order(ModelUnit &pep)
{
    ...
    for (int c=0;c<pep.chains.size();c++)
    {
        ...
        map<int,ResidueUnit> ResidueMap;
        int size=pep.chains[c].residues.size();
        for(int r=0;r<pep.chains[c].residues.size();r++){
            ResidueMap[pep.chains[c].residues[r].resi]=pep.chains[c].residues[r];
        }
        vector<ResidueUnit>().swap(pep.chains[c].residues);
        for(auto iter=ResidueMap.begin();iter!=ResidueMap.end();iter++){
            pep.chains[c].residues.push_back(iter->second);
        }
        map<int,ResidueUnit>().swap(ResidueMap);
    }  
} 

void standardize_residue_order(vector<ResidueUnit> &residues){
    map<int,ResidueUnit> ResidueMap;
    //so there is only one residue for each number
    int size=residues.size();
    for(int r=0;r<residues.size();r++){
        ResidueMap[residues[r].resi]=residues[r];
    }
    vector<ResidueUnit>().swap(residues);
    for(auto iter=ResidueMap.begin();iter!=ResidueMap.end();iter++){
        residues.push_back(iter->second);
    }
    map<int,ResidueUnit>().swap(ResidueMap);
}

void standardize_residue_order(ChainUnit &chain){
    map<int,ResidueUnit> ResidueMap;
    int size=chain.residues.size();
    for(int r=0;r<chain.residues.size();r++){
        ResidueMap[chain.residues[r].resi]=chain.residues[r];
    }
    vector<ResidueUnit>().swap(chain.residues);
    for(auto iter=ResidueMap.begin();iter!=ResidueMap.end();iter++){
        chain.residues.push_back(iter->second);
    }
    map<int,ResidueUnit>().swap(ResidueMap);
}

void standardize_residue_order(ModelUnit &pep){
    for (int c=0;c<pep.chains.size();c++){
        map<int,ResidueUnit> ResidueMap;
        int size=pep.chains[c].residues.size();
        for(int r=0;r<pep.chains[c].residues.size();r++){
...
void standardize_atom_order(ResidueUnit &residue){
    map< string, map< string, int> > NTPs;
    map< string, int> A;
    ...
    AtomUnit temp_atom; 
    temp_atom.xyz.assign(3,0);
    map<int,AtomUnit> temp_atoms;
    for(int a=0;a<residue.atoms.size();a++){
        int order = NTPs[residue.resn][residue.atoms[a].name];
        temp_atom.xyz[0] = residue.atoms[a].xyz[0];
        temp_atom.xyz[1] = residue.atoms[a].xyz[1];
        temp_atom.xyz[2] = residue.atoms[a].xyz[2];
        temp_atom.name = residue.atoms[a].name;
        temp_atom.movable = residue.atoms[a].movable;
        temp_atoms.insert(pair<int,AtomUnit>(order,temp_atom));
    }
    vector<AtomUnit>().swap(residue.atoms);
    for(auto iter:temp_atoms) residue.atoms.push_back(iter.second);
    map<int,AtomUnit>().swap(temp_atoms);
}
...
void pdb2fasta(const char *filename,ModelUnit &pep,const string PDBid="",const int ShowSeqLen=0){
    if (strcmp(filename,"-")==0)
        cout<<pdb2fasta(pep,PDBid,ShowSeqLen);
    else
    {
        ofstream fp(filename);
        fp<<pdb2fasta(pep,PDBid,ShowSeqLen);
        fp.close();
    }
}
