...
/* Check if residue r is adjacent to residue r+1*/
bool isAdjacent(const ModelUnit &pep, const int c, const int r){
    if(pep.chains[c].residues[r].resi+1==pep.chains[c].residues[r+1].resi) return true;
    else return false;
}
...
{
    for (int a=0;a<=12;a++){
        if(pep.chains[c].residues[r].atoms.size()<=a || pep.chains[c].residues[r+1].atoms.size()<=a) break;//previous version: pep.chains[c].residues[r].atoms.size()<=a || pep.chains[c].residues[r+1].atoms.size()
        else if (Points2Distance(
...
int fix_bond_lengths (ModelUnit &pep, double tolerance){
    ...
    for (int c=0; c<pep.chains.size(); c++)
    {
        ...
        for (int r=0; r<pep.chains[c].residues.size(); r++)
        {
            ...
            if (r != (pep.chains[c].residues.size()-1) && isAdjacent(pep,c,r) &&
                      pep.chains[c].residues[r].atoms.size()>=8 && 
                      pep.chains[c].residues[r+1].atoms.size()>=8)
            {
                for (int a1=7;a1<=8;a1++)
                {
                    ...
                    for (a2=0;a2<=3;a2++)
                    {
                        ...
                        if (diff >= tolerance && b>0.0001)
                        {
                            ...
                            if (pep.chains[c].residues[r].atoms[a1].movable==1 &&
                                pep.chains[c].residues[r+1].atoms[a2].movable==0)
                            {
                            ...
                            else if (pep.chains[c].residues[r].atoms[a1].movable==0 && 
                                     pep.chains[c].residues[r+1].atoms[a2].movable==1)
                            {
                            ...
                            else if (pep.chains[c].residues[r].atoms[a1].movable==1 && 
                                     pep.chains[c].residues[r+1].atoms[a2].movable==1)
                            {
            ...
            for (int a1=0; a1<pep.chains[c].residues[r].atoms.size(); a1++)
            {
                ...
                int i1;
                if(pep.chains[c].residues[r].atoms.size()<12) i1=N[atom1];
                else i1=a1;
                for (a2=a1+1; a2<pep.chains[c].residues[r].atoms.size(); a2++)
                {
                    string atom2 = pep.chains[c].residues[r].atoms[a2].name;
                    int i2;
                    if(pep.chains[c].residues[r].atoms.size()<12) i2=N[atom2];
                    else i2=a2;
                    //cout<<r<<" "<<a1<<" "<<a2<<" "<<i1<<" "<<i2<<endl;
                    bool pass = false;
                    // if neither atom is movable, immediately continue
                    if (pep.chains[c].residues[r].atoms[a1].movable==0 && 
                        pep.chains[c].residues[r].atoms[a2].movable==0)
                    {
                    ...
                    if (resname == "  A")
                    {
                        ...
                        {
                            pass = true;
                            b_ideal = A_bonds[i1][i2];
                        }
                        else b_ideal = A_vdw[i1] + A_vdw[i2];
                    } 
                    else if (resname == "  C")
                    {
                        if (C_bonds[i1][i2]>0)
                        {
                            pass = true;
                            b_ideal = C_bonds[i1][i2];
                        }
                        else b_ideal = C_vdw[i1] + C_vdw[i2];
                    } 
                    else if (resname == "  G")
                    {
                        if (G_bonds[i1][i2]>0)
                        {
                            pass = true;
                            b_ideal = G_bonds[i1][i2];
                        }
                        else b_ideal = G_vdw[i1] + G_vdw[i2];
                    } 
                    else if (resname == "  U")
                    {
                        if (U_bonds[i1][i2]>0)
                        {
                            pass = true;
                            b_ideal = U_bonds[i1][i2];
                        }
                        else b_ideal = U_vdw[i1] + U_vdw[i2];
                    }

                    if (! pass && i1>=11 && i2>=11) continue;
                    // Get (x,y,z) coordinates
                    a1x = pep.chains[c].residues[r].atoms[a1].xyz[0];
                    a1y = pep.chains[c].residues[r].atoms[a1].xyz[1];
                    a1z = pep.chains[c].residues[r].atoms[a1].xyz[2];
                    ...
                    b = sqrt((a2x-a1x)*(a2x-a1x) + (a2y-a1y)*(a2y-a1y) + (a2z-a1z)*(a2z-a1z));
                    // Calculate fractional difference between the current and ideal bond lengths
                    if (pass) // bonded atom pair
                    {
                        diff = fabs(b - b_ideal) / b_ideal;
                        deltaB = b - b_ideal; // difference in bond length from ideal
                    }
                    else // unbonded atom pair
                    {
                        diff = b_ideal - b;
                        deltaB = b - (b_ideal - tolerance);
                    }
                        
                    // Check if difference is greater than or equal to the allowed tolerance
                    if ((pass && diff >= tolerance && b>0.0001) ||
                       (!pass && diff >0.4 && deltaB<0 && b>0.0001))
                    {
                        //cout << resname << resnumber << atom1 << atom2 << b << ' ' << b_ideal << ' ' << diff <<endl;
                        // Check which atoms are movable and assign new coordinates to the movable atom
                        if (pep.chains[c].residues[r].atoms[a1].movable==1 && 
                            pep.chains[c].residues[r].atoms[a2].movable==0)
                        {
                            ...
                            pep.chains[c].residues[r].atoms[a1].xyz[2] = (deltaB/b)*(a2z-a1z) + a1z;
                                 ...
                                 pep.chains[c].residues[r].atoms[a2].movable==1)
                        {
                            //cout << "a2 movable " << resname << resnumber << atom1 << atom2 << "b: " << b << "b_ideal: " << b_ideal << "diff: " << diff << endl;
                            moved++;
